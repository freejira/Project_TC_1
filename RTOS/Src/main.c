...
/* USER CODE BEGIN PV */
#define ACS_SENS_MV_PER_A   185   // (±5A 변종)
#define DIV_NUM             1     // 분압기 없음 (VCC=3.3V, OUT 직결)
#define DIV_DEN             1
static uint16_t g_zero_raw = 0;
/* USER CODE END PV */
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	...
	int32_t d_raw =  - (int32_t)raw + (int32_t)g_zero_raw;
	int32_t mv    = (d_raw * 3300) / 4095;
	int32_t ma    = (mv * 1000) / ACS_SENS_MV_PER_A;

	char msg[64];
	int n = snprintf(msg, sizeof(msg), "raw=%4u  d=%ld  I=%ld mA\r\n", raw, d_raw, ma);
