...
** 섹션 목록 **
  ...
  - status   : drowny(writer) -> ai_GUI(reader)
      EAR / 졸음 단계(stage) / 얼굴 검출 여부 / 눈 감김 지속시간 / timestamp
"""
...
class StatusWriter:
    ...
    def write(self, ear, stage, face_detected, closed_duration_s, timestamp):
        d = self._data.status
        d.seq += 1
        d.timestamp = timestamp
        d.ear = ear
        d.face_detected = int(bool(face_detected))
        d.stage = stage
