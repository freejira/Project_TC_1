...
class SysShmClient:
    ...
    def set_flags(self, warning: bool, sleep: bool):
        self._lock()
        try:
            self._data.warning_flag = 1 if warning else 0
            self._data.sleep_flag = 1 if sleep else 0
        finally:
            self._unlock()
...
def main():
    ...
    try:
        while _running:
            ...
            # 시스템 공유 메모리(init.c)에도 매 tick 반영
            sys_shm.set_flags(warning=is_warning, sleep=is_drowsy)
